...
struct floor_stat statistics[NUM_FLOOR] = {
...
void update_floor_stats(int floor_id){
  ...
  for(i = 0; i < NUM_FLOOR; i++){
    ...
    // and increment its calls
    if(statistics[i].floor_id == floor_id){
      statistics[i].calls++;
      break;
    }
  }
  ...
  // Sort the structure
  // i is the currently incremented floor
  // check every floor before i and sort accordingly
  for(int k = i-1; k >=0; k--){
    if(statistics[k].calls < statistics[i].calls){
      struct floor_stat tmp = statistics[i];
      statistics[i] = statistics[k];
      statistics[k] = tmp;
      i--;
      k--;
    }else{
...
int empty_external(struct buttons *buttons){
  for (int i = 0; i < NUM_FLOOR; i++){
    if(buttons->out[i][0] > 0 || buttons->out[i][1] > 0){
      return -1;
    }
  }
  return 0;
...
void lift_process_states(struct lift *lift, struct buttons *buttons){
  ...
  for(int i=0; i < NUM_LIFT; i++){
    ...
    switch(lift[i].state){
      ...
      }else if(empty_internal(i, buttons) == 0 && empty_external(buttons) == 0){
          // no internal request
          if(fabs(lift[i].floor - statistics[i].floor_id) <= LIFT_FLOOR_ERROR_EPS){
            lift_set_operation(&lift[i], LIFT_STOPPING);
            lift[i].floor = statistics[i].floor_id;
            // don't continue moving the lift, break the LIFT_MOVING
            // process
            break;
          }
        ...
        switch(lift[i].door){
          ...
          case DOOR_CLOSED:
            ...
            if(lift[i].dir != DIR_NONE){
              ...
              if(hasRequest == 0 && FLOOR(lift[i].floor) != statistics[i].floor_id && empty_external(buttons) == 0){
                // Lift is currently inactive
                // Move it towards most frequent floors
                lift[i].dir = FLOOR(lift[i].floor) > statistics[i].floor_id ?  DIR_DOWN : DIR_UP;
                lift_set_operation(&lift[i], LIFT_STARTING);
              }
